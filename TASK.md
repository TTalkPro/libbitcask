...
- [x] **S29-11-②④ 已完成(2026-07-13,S32 之后开工;前置召回基建由
  S32-M0c 清偿)**:
  - **② AVX2 int8 内核**(`int8_kernels.hpp::dot_avx2`):vpmaddubsw +
    vpmaddwd 经典两步;**sign 技巧防饱和**(偏置法 u8×s8 对和上界
    64770 > s16 → 静默饱和;|d|⊙sign(q,d) 上界 32258 < 32767 恒安全)。
    分发链 VNNI512 → VNNI256 → **AVX2** → nullptr——非 VNNI 机器
    (2013 Haswell 起)全 int8 路径激活(查询两阶段/inmem_int8 非降级/
    IVF 扫描/DiskANN 建图共用)。契约 = **整数域与标量精确一致**
    (新增 Int8Kernel.SimdMatchesScalarExactly:全值域±极值 + 奇数维
    尾部;float 侧 ≤4 ULP——target 属性下 FMA contraction 舍入路径
    不同,1 ULP 实测,top-k 排序不受影响由三引擎穷举对拍守护)。
  - **② 混合精度建图导航**(`HnswConfig::build_nav_int8`,默认开,
    kDot+内核在时生效;`hnsw_build_nav_int8` 配置链回退闸):导航
    (上层贪心+逐层 ef 搜索)走 int8(工作集 153MB→38MB@100k×384d),
    **入选邻居 f32 重算精选**(ef 候选重算 ≪ 导航全程)。
  - **实测(无 VNNI 机,空载)**:插入 10k **3.0k→3.86k(+29%)**,
    100k **1.2k→2.09k docs/s(+75%,82s→48s)**;超线性 2.5×→1.85×
    (38MB 仍 > L3 24MB,理论一致——更高维收益更大);聚簇语料 build
    +20%;**recall@10 = 1.000 零损失**(f32 精选兜底,验收门通过);
    IVF/查询延迟中性(GCC 已自动向量化标量,AVX2 收益为保证性)。
...
- [x] **M5 已完成(2026-07-13,用户拍板提前启动)DiskANN 引擎 v1——
  定级「实验性」**:
  - **DiskannSegment**(`diskann.hpp/.cpp`,BDA1 格式):Vamana 单层图
    (增量建图 ×2 趟 + RobustPrune α=1.2 + 反向边回剪 + 孤点救援)+
    盘上节点块共置 [ord|int8 码|邻接 R]（mmap 随机读,「一次块读取到
    距离与下一跳」）+ RAM 导航码(v1 = int8 三件套,est ≡ 精评;
    **PQ32 压缩是 M5.5 核心目标**——届时才兑现「RAM ≪ N·dim」)。
    beam search 候选池 L(ef 语义;L=count 全图精确退化——对拍原则)。
  - **DiskannPlugin**:结构复刻 IvfPlugin(sealed 段 + HnswIndex 窗口 +
    DeltaLog + 组件链 diskann.ckpt/kDiskann 段/diskann.bda);工厂/
    meta/component_of_plugin/vec_engine_migrate --to diskann 全接线,
    三引擎三向切换冒烟通过。
  - **调试轨迹(4 个发现,全部有数据)**:① 距离域 bug——RobustPrune 的
    α 缩放假设距离非负,-dot 负值域使覆盖判据反向 → 过度剪枝
    (l=64 召回 0.58,est 换精确分无效即图坏证据);改余弦距离 1-dot。
    ② 单趟建图 100k 崩(0.59)——早插入点边基于残缺图;改两趟。
    ③ 论文 α 调度(1.0→1.2)在余弦紧簇域更糟(α=1.0 边界被簇内微距
    噪声主导 → 断图 recall 归零);两趟同 α=1.2。**α 调度对度量域
    敏感,非普适**。④ 极端二态语料(v2 合成:781 等距孤簇,簇内三角
    扁平)的结构性病态:近序贪心填边被 ~128 个簇内候选耗尽全部 R 槽,
    跨簇长边永远轮不到 → 图成孤岛(HNSW 层级/IVF 质心路由结构性免疫,
    单层图正中要害;文献语料 SIFT/DEEP 均连续分布)。补丁:分层多起点
    (16 入口,build/search 共用)+ 桥边配额(近序填 R−q,保 q=R/8 最远
    候选)——0 → 0.62@L128,仍不达标。
  - **定级依据(对照实验)**:连续距离分布(弱聚簇 σ=0.5,20k)下
    L=64 召回 ≥0.85 达标——失败限于二态合成语料,非引擎缺陷。
    **v1 = 实验性:真实语料(qwen3 嵌入)验证通过前不建议生产选用**
    (IVF 引擎在同尺度同语料 recall@10_i8=1.0,是磁盘档默认推荐)。
  - 验收:段 5 测(对拍/召回门/连续分布对照/守卫/空段)+ 插件 3 测 +
    Cask e2e + 工具三向冒烟——gcc 全量 **640/640**;ASan 7/7;
    TSan 7/7;build-rel 过。
- [ ] **M5.5(挂账)DiskANN 生产化**:① 导航码压缩——**PQ32 vs RaBitQ
  正版对比评估**(PQ32:32B/向量但需码本训练管线;RaBitQ 正版:136B@1024d、
  零训练、~1.9% 可证误差界——lite→正版四步升级路径与理论推导见
  [`doc/rabitq-theory-zh.md`](doc/rabitq-theory-zh.md) §8;由召回 harness
  出数定,RAM 10GB→320MB/1.4GB@10M×1024d);② 真实语料召回验证(解除

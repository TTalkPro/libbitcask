...
| **W5** | Windows CRT/堆 边界（/MT 下每模块各一份）| 🔍 S37-6 引入配置期告警。**准确范围：本库的 C++ API 本就不是模块边界，只有 C API 是** —— `io::File::pread` 返回 `std::vector`（库这侧分配、调用方析构）、`io::open_stream` 返回 `FILE*`（同理）、`io::flush_and_sync_stream` 反向收 `FILE*`。这些在 `/MD`（单份共享 CRT，本项目默认）下全部成立，在 `/MT` + 多模块下则是跨堆 free 与跨 fd 表查号。后者的失败形态是 **invalid-parameter handler → `__fastfail`，进程无声消失、退出码 0xC0000409、terminate/abort 钩子都不触发**（参考 `../coxswain` W9.9，当时靠 cdb 取栈才定位；该状态码名为 STACK_BUFFER_OVERRUN，与栈无关）。当前默认 `-MD` 故不触发。**待办**：在 README/`doc/api-c.md` 写明「C++ 头要求与库同模块、同 CRT；跨模块只用 C API」，并考虑把 `flush_and_sync_stream` 改成收句柄而非 `FILE*`（去掉最刺眼的一处）|

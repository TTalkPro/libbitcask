...
struct CaskOptions {
    ...
    //                            （约一半，**夹在 [64, 1024]**——S33-6 加的绝对
    //                            上限：rlimit 一大时"一半"等于没有上限），开箱即
    //                            防大库无界累积 fd/mmap 撞 `ulimit -n` /
    //                            `vm.max_map_count`；
...
public:
    ...
    /// 自动档（`max_read_handles == 0`）的上下界。S33-6 加绝对上限：rlimit
    /// 一大（容器/systemd 常见 5×10^5+），"取一半"等于没有上限。
    static constexpr std::size_t kAutoReadHandleFloor   = 64;
    static constexpr std::size_t kAutoReadHandleCeiling = 1024;
    ...
    // S35：跨崩溃原子批（doc/atomic-batch-design-zh.md）。语义 = put_batch
    // 的超集：除进程内 all-or-nothing 外，**崩溃/掉电后整批要么全可见要么
    // 全不可见**（盘上批头声明区间，恢复时区间不完整 ⟹ 整批截断）。
    // - 支持批内 REMOVE（kRemove 忽略 value；墓碑成员 v0 编码）。
    // - 批内 op 依序 apply（同 key 多次 = 批内 LWW）。
    // - **首次调用把目录 meta 懒升级为 v6**——此后 5.1.0 及更老读端拒开
    //   该目录（unsupported meta version）。从不调用本方法的目录停留 v5。

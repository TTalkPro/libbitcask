...
//
// === 结构 ===
//   sealed_：不可变 IvfSegment（mmap，簇 posting 顺序扫；ivf.biv 侧车）
...
//   base  = ivf.ckpt（kIvf 段：gen/count/dim/nlist 交叉校验）+ ivf.biv
//           侧车（IvfSegment::build 全量重建：sealed 活集 + window 活集
//           → 重训重分簇；死清理内建——查询侧 live 过滤兜到下次 base）。
//   delta = ivf.ckpt.d<seq>（kDeltaInfo + kHnswDelta 通用向量插入日志），
//           恢复重放进 window（重放 = 窗口重建图，代价由 rebase_min_docs
//           双门槛围住——S32-M1 同款）。
//   崩溃窗口：新 ivf.biv 已 rename 而新 ckpt 未落 → gen 守卫拒载 → fold
//           全量重建（与 HNSW payload 同语义）。
//
// === 与 HNSW 引擎的语义差 ===
//   rebuild()（merge 收尾）不做 eager 重建——盘上段死清理推迟到下次
//   base（查询侧 live 已滤，无正确性差；HNSW 必须 eager 是因为死节点
//   占 RAM 图）。search 的 ef 参数按 nprobe 解释（0 = 自动）。
//
// === 线程模型 ===
...
class IvfPlugin final : public VectorEnginePlugin {
public:
    IvfPlugin(const VectorPluginConfig& config, const bm25::DocTable& docs);
    ...
    // ---- plugin::CaskPlugin ----
    [[nodiscard]] std::string_view name() const override { return "ivfrq"; }
...
private:
    ...
    std::atomic<std::shared_ptr<const IvfSegment>> sealed_;

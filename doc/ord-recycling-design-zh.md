...
`meta_blobs_[ord]`）随 `next_ord_` 单调增长，死 slot 占据内存。
（注：方案 B 落地后，`slots_`/`ord2ext_` 已分块、**全死 chunk 可在 merge 释放**；
`live_`/`doc_lens_` 仍平坦常驻。详见 §5。下文为方案 A/B 的完整可行性分析。）
...
| **幂等水位** | `hnsw.cpp:653` `ord <= max_inserted_ord_ → reject`；`inverted.cpp:323` 同理 | ord 永远递增，小 ord = 已处理 | 回收 ord 被水位拦截 → **静默丢弃** |
...
| # | 组件 | 不变量 | 文件:行 | 回收后破坏方式 |
|---|------|--------|---------|---------------|
| 1 | HNSW | `ord <= max_inserted_ord_` → 拒绝插入 | `hnsw.cpp:653-656` | 回收 ord ≤ 水位 → 向量被静默丢弃 |
| 2 | HNSW 快照 | ord 在快照中严格递增（V7:BVH2 v2） | `hnsw.cpp:1371` | 快照加载失败（整体拒绝） |
| 3 | HNSW | `NodeChunk::ords[slot]` = ord | `hnsw.cpp:713,1192` | 并发读者看到 stale ord |
...
| 7 | Inverted | `PostingBlock` base_ord/end_ord 范围 | `inverted.hpp:72-73` | WAND 块跳过误判 |
| 8 | Inverted WAL | WAL 条目存原始 ord | `inverted_wal.cpp:120` | 重放回收 ord 触发水位拦截 |
| 9 | Inverted load | max_indexed_ord_ 从最大 posting ord 重建 | `inverted.cpp:2007-2009` | 快照后回收 ord 被重放拒绝 |
| 10 | Index | 所有数组 `[ord]` | `index.hpp:133-145` | 回收 ord ≥ 数组长度 → 越界 |
| 11 | Index | `put_doc`: 旧 ord 软删，新 ord 占 slot | `index.cpp:65-84` | slot 未清 → stale 数据 |
| 12 | Index | `for_each_live`: 0..size 扫描 live_ | `index.hpp:140` | 回收 ord 重复出现 → 双重计数 |
| 13 | Index | `fill_is_live`/`fill_doc_lens` SIMD 假设 in-bounds | `index.cpp:176,206` | 慢路径处理越界；快路径静默出错 |
...
> ✅ **本方案已落地**（与下方设计基本一致）。实现：`Index` 的 `slots_` / `ord2ext_`
> 改为 `std::vector<std::unique_ptr<Chunk>> chunks_`，`kChunkOrds = 65536`，`Chunk` 带
> `live_count`（`index.hpp:64-69`）；`compact_chunks()`（`index.cpp:252`）在 merge 后
> （`cask.cpp:1768` `compact_index_chunks`）释放 `live_count==0` 的 chunk。**偏差**：
> `live_` / `doc_lens_` **保持平坦**（不分块），以兼容 `fill_is_live` 的 SIMD gather
> （`index.hpp:62`）——故为「slots/ord2ext 分块 + live/doc_lens 平坦」的混合形态。
...
```
 Delete 时间点: live_[ord] = false (软删除)，postings 和 HNSW 节点仍在
                    ↓
 Merge Phase 2: compact(0.2) — 按阈值压实死 posting（不重读、不重分词）；
                compact_index_chunks() — 释放全死 DocSlot chunk（cask.cpp:1767-1768）
 Merge Phase 2: rebuild_hnsw() — 全量图重建，跳过死节点 → 死节点物理消失
                    ↓
...
> **更新**：本节的建议「先走方案 B」**已落地**——`Index` 的 `slots_`/`ord2ext_` 已分块、
> merge 后释放全死 chunk（见 §5 顶部）。方案 A（seq + ord 回收）仍**未做**，结论不变。
...
`Cask::put()` 每次调用都 `alloc_ord()` 分配新 ord（`cask.cpp:1373`）。
`Cask::remove()` 也分配新 ord 作为墓碑标识（`cask.cpp:1430`）。
...
`add_doc` 的实现是 `pl.items.push_back({ord, tf, ...})`（`inverted.cpp:316` 起）—— 纯尾部追加，
...
HNSW（`hnsw.cpp:653`）和 InvertedIndex（`inverted.cpp:323`）都用水位 `ord <= max_X_ord_ → 拒绝`，
...
put("foo") 更新 → ord=0 标死
  ...
  ↓ compact(): dead_ratio ≥ 阈值时 compact_flags() 物理移除死 posting
  ↓ merge 时也调 compact(0.2)（cask.cpp:1767）压实死 posting；不再全量 rebuild_index
```
...
| search_phrase | `inverted.cpp:757` | `fill_is_live(first_ords, first_live)` → 候选检查 |
| bool_search BMW | `inverted.cpp:1050` | 块级惰性 fill，`ensure_block()` |
| bool_search must/should | `inverted.cpp:1170` | 每 term 批量 fill |
...
**关键点（已随 V4 merge 重构更新）：merge 管线已包含 `compact(0.2)`**（`cask.cpp:1767`），
不再依赖全量 `rebuild_index()`。无后台线程自动触发；若用户既不手动 `compact()`、又不 merge，
posting list 会随写入膨胀。

### 10.4 `fill_is_live` 成本

`Index::fill_is_live`（`index.cpp:176`）—— 批量取 live 状态：
...
| ~~**merge 时加 compact 步**~~（**已实现**） | merge 管线已直接调 `compact(0.2)`（`cask.cpp:1767`） | merge 即回收死 posting |

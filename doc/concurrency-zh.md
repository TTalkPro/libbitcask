...
> **另一个正交层面：单个 handle 的多线程安全**（S11，通用 C++ 库定位）。同一个
> `Cask` 对象可被多线程安全共享：读并发无锁、写由内部 `write_mu_` 串行化、读写
> 并发安全、close fail-fast、`parallel_scan` 并行全表扫描。完整契约见
> [`design/thread-safety.md`](design/thread-safety.md)；本文聚焦的是「多个 handle /
> 多进程共享同一目录」这个不同的轴。
...
| HnswIndex      | ✅ 是      | per-node 自旋锁 + `atomic<NodeChunk*>` 无锁发布 |
...
服务多个调用线程的并发读写。S11（通用 C++ 库定位）后，**同一 handle 即可被多线程
安全共享**——读并发无锁、写由内部 `write_mu_` 串行化（详见
[`design/thread-safety.md`](design/thread-safety.md)）。配置上：

- 一个 handle：`bitcask_open(Dir, &opts, &cask, &fault)`，`opts.read_write = 1`，
  多个调用线程**直接共享**它做并发 get/put/search（无需每线程一个 handle）。
- 应用层用独立线程或 cron 触发周期 `bitcask_merge(cask, &fault)`

运行期间：

- 全部读看到最新写入（KeyDir 共享 + shared_mutex；搜索 near-real-time）
- **多线程并发写同一 handle 安全**（S11-W1：内部 `write_mu_` 串行化，不再要求
  caller 串行化）。写在文件层本就串行 → 锁不损吞吐；**更高写并发 → 按目录分片
  多个实例**（单 append WAL 的横向扩展手段）。
- merge 跟读写并行不阻塞（独立 merge.lock + keydir shared_mutex 协调）
...
| 模块 | 锁名称 | 类型 | 声明位置 | 说明 |
|------|--------|------|----------|------|
| **KeyDir** | barrier_mu_ | std::mutex | include/bitcask/keydir.hpp:430 | 屏障间互斥，跨整个屏障持有 |
| **KeyDir** | gate_mu_ | std::mutex | include/bitcask/keydir.hpp:431 | 写者退避等待的 cv 配套锁 |
| **KeyDir** | gate_cv_ | std::condition_variable | include/bitcask/keydir.hpp:432 | 写者退避等待的条件变量 |
...
- 方向：shard → meta（与全序一致）
- 代码位置：
  - get: src/keydir/keydir.cpp:300-361（meta 嵌套 :316）
  - put: src/keydir/keydir.cpp:363-435（meta 嵌套 :426）
  - remove: src/keydir/keydir.cpp:617-709（meta 嵌套 :675）

**屏障操作规则（start/release/save_snapshot/load_snapshot）：**
- 通过 BarrierGuard RAII 类自动管理屏障生命周期
- 屏障期间写者被闸门出清，读者照常并发
- 屏障内不持分片锁遍历各分片 entries（读-读并发安全）
- 代码位置：src/keydir/keydir.cpp:138-166（BarrierGuard 类；前置注释 :108-137）
...
**代码位置（meta_mu_ 嵌套获取处）：**
- src/keydir/keydir.cpp:316（get，shared）
- src/keydir/keydir.cpp:426（put，unique）
- src/keydir/keydir.cpp:675（remove，unique）
...
**代码位置：**
- src/keydir/keydir.cpp:918-931（函数 :918，meta shared :919，嵌套分片锁 :931）
...
**代码位置：** src/keydir/keydir.cpp:711-753
...
**代码证据：**
- src/keydir/keydir.cpp:617-709（remove 内部重新检查命中状态）
- src/keydir/keydir.cpp:753（conditional_remove 末尾 remove() 返回值统一为 kOk）
...
**代码证据：**
- src/keydir/keydir.cpp:108-137（BarrierGuard 注释）
- include/bitcask/keydir.hpp:15-34（锁全序注释）
...
**代码证据：**
- include/bitcask/keydir.hpp:367
...
**代码证据：**
- include/bitcask/keydir.hpp:371（Shard::mu 注释）
...
**代码证据：**
- src/keydir/keydir.cpp:286-297（get 查找顺序注释）
- src/keydir/keydir.cpp:385-428（put 探测逻辑）

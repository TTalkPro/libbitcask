...
**落地勘误（2026-07-13，S32-M5 v1，定级「实验性」）**：实际落地 ~1.1k LOC
（S32 基建摊薄：窗口/链/DeltaLog/工厂/工具全复用）。v1 裁剪：RAM 导航码 =
int8 三件套（非 PQ32——「RAM ≪ N·dim」尚未兑现，PQ 是 M5.5 核心）；盘上
int8（非 f32）。四个实现发现（详 TASK.md M5）：α-prune 距离域必须非负
（余弦用 1−dot）；两趟建图必需；**α 调度（1.0→1.2）对度量域敏感非普适**；
极端二态聚簇语料是单层图的结构性病态（近序贪心填边被簇内候选耗尽 R 槽 →
孤岛；多起点+桥边配额缓解不治本），连续分布对照达标 → 真实语料验证是
解除「实验性」的前置。当前磁盘档默认推荐 = ivfrq（同语料 recall@10_i8=1.0）。

...
| **删除困难** | 只能软删除（`mark_deleted` 留死节点当路标），物理清除需 merge 重建（`hnsw.hpp:38-40`） |
...
- `save()` / `load()`：BCVS 格式（`[magic][ver][payload][crc32]`），`tmp+rename` 原子落盘（`hnsw.hpp:116-138`）。

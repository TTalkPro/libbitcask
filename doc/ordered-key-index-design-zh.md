...
与 5.0.0 tstamp64 同模式：新旧互相干净拒开，离线迁移一次通过。库版本 bump 到 6.0.0（盘上 `bitcask.meta` = v5）。
...
S33-2 是发布边界：它单独构成 6.0.0 的 flag-day（可先发，OKI 功能后续小版本跟进），也可与 S33-3..6 合并一次发布——取决于停机窗口安排，倾向合并（一次停机拿到完整 Range 能力）。

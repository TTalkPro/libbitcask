...
- `kFlagVecQuantized = 0x08`：向量段是码字而非裸 f32（量化由该 **flag 位**标识，
  **无独立 magic**——文中早期「kQuantizedMagic "QCOD"」stub 已不存在）。
- 码字内含 **`kQuantizedVersion` 单字节**（scheme 版本），见下方布局。
- 向量段定序最前：`[Dim:varint][ f32×Dim 或 量化码字 ]`，便于 HNSW O(1) 切片。

P3 = 真正的 int8 编解码（已落地，见 `codec.cpp` 的 `encode_doc_value` 量化分支）。
...
- **落盘码字布局**（kFlagVecQuantized 置位时的向量段）：
  ```
  [Dim:varint][SchemeVer:u8 = kQuantizedVersion][scale:f32 LE][int8 × Dim]
  ```
  大小 = varint(Dim) + 1 + 4 + Dim 字节。复用同一量化器 → 落盘 int8 可**直接喂给
...
接近瓶颈。**已落地（P5b）**：`HnswConfig::inmem_int8` / `CaskOptions::vector_inmem_int8`
（去常驻 f32 副本 + 查询侧量化，仅 kDot，kL2 拒绝）——详见

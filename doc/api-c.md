...
| 产物 | 说明 |
|------|------|
| `libbitcask.so` | 共享库，导出全部 C API（`SOVERSION=6`，`VERSION=6.3.1`，由 `CMakeLists.txt` 的 `project(libbitcask VERSION 6.3.1)` 单一真源派生）|
...
版本号由 `CMakeLists.txt` 的 `project(libbitcask VERSION 6.3.1)` 单一真源派生，configure 时通过 `c_api/bitcask_version.h.in` 生成 `bitcask_version.h`。
...
- 运行时返回值与 `libbitcask.so.6.3.1` 文件名完全对应；SOVERSION 是 `6`（大版本号），反映 ABI 兼容性。

...
- `bitcask_put_batch_atomic`：引擎原子批直通——崩溃/掉电后整批要么全可见要么全不可见；批内 op 依序 apply（同 key 多次 = 批内 LWW，不校验重复）。**首次调用把目录 meta 懒升级为 v6**（此后 5.1.0 及更老读端拒开该目录；从不调用则停留 v5）。durability 同 `bitcask_put_batch`。

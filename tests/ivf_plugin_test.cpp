// IvfPlugin 插件级测试（S32-M3）：base/delta 链轮回、窗口→段换代、删除
...
vec::VectorPluginConfig ivf_cfg(std::uint16_t dim) {
...
TEST(IvfPlugin, BaseDeltaReopenRoundTrip) {
    const fs::path dir = fs::temp_directory_path() / "bitcask_ivfp_rt";
    fs::remove_all(dir);
    ...
    FakeDocTable dt;
    vec::IvfPlugin a(ivf_cfg(dim), dt);
    ...
    EXPECT_TRUE(fs::exists(dir / "ivf.ckpt"));
    EXPECT_TRUE(fs::exists(dir / "ivf.biv"));
    ...
    EXPECT_TRUE(fs::exists(dir / "ivf.ckpt.d1"));
    EXPECT_EQ(a.sealed_size(), 4u);
    ...
    vec::IvfPlugin b(ivf_cfg(dim), dt);
...
TEST(IvfPlugin, RebaseDropsDeadPhysically) {
    const fs::path dir = fs::temp_directory_path() / "bitcask_ivfp_dead";
    fs::remove_all(dir);
    fs::create_directories(dir);
    const std::uint16_t dim = 8;

    FakeDocTable dt;
    vec::IvfPlugin p(ivf_cfg(dim), dt);
    plugin::OpenContext ctx;
    const std::string dir_s = dir.string();
    ctx.dir = dir_s;
...
TEST(IvfPlugin, RebaseMinDocsWindow) {
    const fs::path dir = fs::temp_directory_path() / "bitcask_ivfp_win";
    fs::remove_all(dir);
    fs::create_directories(dir);
    const std::uint16_t dim = 8;

    FakeDocTable dt;
    auto cfg = ivf_cfg(dim);
    cfg.rebase_min_docs = 3;
    vec::IvfPlugin p(cfg, dt);
    plugin::OpenContext ctx;
    const std::string dir_s = dir.string();
    ctx.dir = dir_s;
    ASSERT_EQ(p.open(ctx), plugin::PluginStatus::kOk);

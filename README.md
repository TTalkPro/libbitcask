...
版本信息由 `CMakeLists.txt` 的 `project(libbitcask VERSION 6.6.0)` 单一真源派生：`libbitcask.so` 的 `SOVERSION=6`，`VERSION=6.6.0`，C 端 `bitcask_version_{major,minor,patch,string}()` 同步。
